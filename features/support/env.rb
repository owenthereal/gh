...
Before do
  ...
  set_env 'GH_API_HOST', '127.0.0.1:0'
...
World Module.new {
  # FIXEM: hack to make all exact match a partial match
  def assert_exact_output(*args)
    assert_partial_output(*args)
  end
  ...
  def edit_hub_config
    ...
    gh_config = File.join(ENV['HOME'], '.config/gh')
    FileUtils.mkdir_p File.dirname(gh_config)
    if File.exist? config
